...
4. PropertySource注解扫描注入时,配置类增加校验注解,是否可为空,是否指定默认值,直接可以打印字段注入异常提示框(已修改为JSON文件作为配置文件)
...
#### 代码优化清单
1. 菜单栏和工具栏代码重构,可以参考编辑框右键菜单的写法，进行代码重构(已完成代码重构)✅
2. 菜单、工具条,LOGO,图标名称全部放到Properties文件，可以参考RSyntaxTextArea组件的右键菜单(已完成代码重构)✅
...
7. 软件启动速度优化(解决启动加载配置时间过长问题)(配置加载方式由Properties反射到实体类,修改为json文件读取为实体类)✅
...
12. 搜索框可以染色标记所有的编辑框✅
13. 文本编辑框左侧的行号栏右侧边线gutterBorder属性颜色显示不生效(解决方案,文本编辑器xml主题应用代码执行顺序导致，应该等文本编辑器所有的属性都应用完毕，再次应用主题，方可解决)✅
14. 设置新的主题或者设置新的字体后，多文本编辑框字体变小，问题处理(原因：文本编辑器xml主题应用代码执行顺序导致字体不生效，应该等文本编辑器所有的属性都应用完毕，再次应用主题，方可解决)✅
...
4. 软件使用统计面板(软件打开次数，格式化次数，保存文件次数，保存图片次数，软件打开时长〔打开到关闭的时间汇总〕)
5. 软件启动界面开发，初始化好数据以后，进入软件操作主界面
6. 软件退出时，保存配置弹出进度条提示框（退出保存配置已开发为jvm钩子）❌
7. 全局字体设置：软件界面字体设置，编辑框字体设置，关闭保存，启动加载✅
8. 锁屏模式(可以设置定时锁屏时间，锁屏后，需要解锁)❌
9. 可以设置分享的报文图片水印，图片水印可以配置（增加个性化）
10. 可以在设置面板中查看日志✅
11. 增加选项卡，增加代码编辑框为6个(增加布局方式：2个时，双联水平，双联垂直，三个时：三联水平，三联垂直，三联堆积，4个时：四联水平，四联垂直，4个时：四联堆积，六个时，六联堆积)
12. 右侧栏目，可增加别的小工具(翻译，当前时间戳，Unicode，Json树结构......)，类似于Idea右侧工具栏
13.自定义弹框增加倒计时显示,增加错误，警告，信息图标增加标题，内容，关闭按钮
